...
Box query[2] = { Bbox( 1.5,1.5,1.75,1.75), Bbox( 2,2,3,3)};
void callback( const Box& a, const Box& b ) {
    std::cout << "box " << a.id() << " intersects box " << b.id() << std::endl;
}
int main() {
    CGAL::box_intersection_d( boxes, boxes+9, query, query+2, callback,std::ptrdiff_t(1),CGAL::Box_intersection_d::HALF_OPEN);
    return 0;
}